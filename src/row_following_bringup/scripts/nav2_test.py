...
class FollowPathClient(Node):
    def __init__(self):
        ...
        # self.create_timer(0.1, self._send_goal)   #Working
    ...
    def _send_goal(self):
        ...
        for angle in range(-90, 90, step):
            ...
            # waypoint_msg_t = rnp.msgify(Transform, waypoint)

            waypoint_msg_pose = rnp.msgify(Pose, waypoint)

            waypoint_msg_pose_stamped = PoseStamped()
            waypoint_msg_pose_stamped.header.frame_id = 'base_link'
            waypoint_msg_pose_stamped.pose = waypoint_msg_pose

            # waypoint_msg_path = Path
            # waypoint_msg_path.header = 'base_link'
            # waypoint_msg_path = waypoint_msg_pose

            # waypoint_msg = waypoint_msg_t

            path_msg.poses.append(waypoint_msg_pose_stamped)

            # self.path_publisher.publish(waypoint_msg_path)

        # path_msg.poses = [path_msg]
